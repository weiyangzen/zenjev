...
| Validation profile | `python3 -m compileall -q jev scripts`; `python3 -m pytest -q`; `python3 scripts/validate_blueprint.py --check`; `python3 scripts/smoke.py`; schema/config lint; deterministic smoke inference; train/EMA/collapse-recovery simulation; NVIDIA GPU CUDA/VRAM/latency benchmark; manifest checksum audit. A missing applicable validator is a failure, not a skip. |
